...
def make_tasks(task_description: str, run_dir: str = "") -> list[Task]:
    ...
    run_dir больше не диктуется в промптах: путь — факт окружения
    (AI_TEAM_RUN_DIR), а не строка, которую модель должна скопировать без ошибки.
    Параметр сохранён для совместимости вызова и будущих фаз.
    """
    ...
    fix_task = Task(
        ...
        - Если тест падает с 404 на endpoint — добавь его в app/main.py
        - Если тест падает с trailing slash ('https://example.com/' vs 'https://example.com') — используй str вместо HttpUrl
        - Если тест падает с неверным статусом redirect — проверь семантику (307 vs 302)
