...
class TestInputBuffer:
    ...
    def test_push_discards_when_full(self):
        ...
        # Use RIGHT as current_direction: UP is not opposite to RIGHT, so pop returns UP
    ...
    def test_pop_ignores_opposite_if_somehow_present(self):
        """ASSERT-10: If a direction opposite to current is in the queue, pop returns current."""
        buf = InputBuffer()
        # Directly append an opposite direction to simulate the theoretical case
        buf._queue.append(Direction.UP)
        # pop with DOWN as current: UP is opposite to DOWN, should return DOWN
        assert buf.pop(Direction.DOWN) == Direction.DOWN

    def test_size_property(self):
        """ASSERT-09: size property reflects the number of queued directions."""
        buf = InputBuffer()
        assert buf.size == 0
        buf.push(Direction.UP, Direction.RIGHT)
        assert buf.size == 1
