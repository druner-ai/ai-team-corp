...
1. **Cache hit → запрос в БД** (src/services/url_shortener.py)  
   Убран запрос к PostgreSQL при попадании в кэш Redis. Теперь кэш хранит JSON с `url` и `expires_at`, проверка истечения срока делается внутри кэша без обращения к БД. При удалении ссылки кэш инвалидируется.

### 🟡 Важные

2. **Некорректная инициализация `Base` для Alembic** (src/models/__init__.py)  
   Заменено на импорт декларативного `Base` из `url_mapping.py`.

3. **Отсутствует импорт `status` в `stats.py`** (src/services/stats.py)  
   Добавлен `from fastapi import status`, используется `status.HTTP_404_NOT_FOUND`.

4. **Ошибка в вычислении `Retry-After`** (src/middleware/rate_limiter.py)  
   Упрощено до статического `retry_after = 60`.

5. **Отсутствие обработки ошибок Redis в rate limiter** (src/middleware/rate_limiter.py)  
   Все вызовы Redis обёрнуты в `try/except`: при недоступности Redis лимитирование пропускается, запрос выполняется.

6. **Некорректное закрытие Redis** (src/main.py)  
   В lifespan‑shutdown теперь вызывается `close_redis_pool()`, которая корректно закрывает пул.

7. **Отсутствие fallback при недоступности Redis** (src/db/redis.py)  
   `create_redis_pool()` при ошибке логирует предупреждение и возвращает `None`. `get_redis()` возвращает `None`, если пула нет. Все сервисы и middleware теперь проверяют наличие `redis` перед использованием (работают без кэша/счётчиков).

8. **Отсутствует импорт `Optional`** (src/schemas/stats.py)  
   Заменено на `datetime | None` (Python 3.11+).

### 🟢 Минорные (исправлены попутно)

- Удалён неиспользуемый импорт `update` в `url_shortener.py`.  
- Убран неиспользуемый параметр `request` в `redirect.py`.  
- `server_default` для Boolean исправлен на `server_default=text("false")`.  
- SSRF‑проверка улучшена: для IP‑адресов используется корректная проверка `ipaddress`, для доменов — точное совпадение.  
- В тестах уточнено ожидаемое исключение (`HTTPException` вместо общего `Exception`).  
- Dockerfile упрощён: одна команда `pip install --no-cache-dir .`.
...
"""
from src.models.url_mapping import UrlMapping, Base

__all__ = ["UrlMapping", "Base"]
```
