...
2. **🔴 Валидация входных данных** — Валидация уже реализована через Pydantic-схемы на всех эндпоинтах (ложное срабатывание QA).
3. **🔴 Обработка ошибок** — Добавлен обработчик `IntegrityError` и улучшен generic-обработчик в `main.py`.
4. **🔴 Тесты на race conditions** — Добавлен тест на конкурентное обновление задачи в `test_update_task.py`.
5. **🟡 XSS-защита** — Добавлена санитизация HTML в Pydantic-валидаторе схемы `TaskCreate`.
6. **🟡 Утечки памяти** — Добавлен `await session.close()` в `finally` блоке `get_db` (уже был, но улучшен).
7. **🟡 Ошибки в сервисном слое** — Добавлена обработка `IntegrityError` в `task_service.py`.
8. **🟡 Ошибки в репозитории** — Добавлен rollback при ошибках коммита в `task_repository.py`.

---

## Полная кодовая база (исправленная)
...
async def get_db() -> AsyncGenerator[AsyncSession, None]:
    ...
    Includes explicit cleanup to prevent memory leaks from unclosed sessions.
...
Includes XSS protection via HTML sanitization on string fields.
"""
...
class TaskCreate(BaseModel):
    ...
    Attributes:
        title: Task description, must be 1-500 characters.
               HTML tags are stripped for XSS protection.
    """
    title: str = Field(
        ...,
        min_length=1,
    ...
    def sanitize_title(cls, v: str) -> str:
        """
        Sanitize the title field to prevent XSS attacks.
        ...
        Returns:
            Sanitized title string with HTML tags removed.
        """
        # Strip all HTML tags to prevent XSS
        cleaned = bleach.clean(v, tags=[], attributes={}, strip=True)
        # Also strip any remaining HTML entities
...
### `app/exceptions/task_exceptions.py`
```python
"""
...
All queries use SQLAlchemy ORM with automatic parameterization
...
class TaskRepository:
    ...
    All database operations include proper error handling with rollback support.
    """
    ...
    async def add(session: AsyncSession, title: str) -> Task:
        ...
        Args:
            ...
            title: Task description (already sanitized by Pydantic schema).
    ...
    async def get_all(session: AsyncSession) -> Sequence[Task]:
        ...
        try:
            ...
            logger.error("Database error retrieving tasks: %s", e)
            raise
    ...
    async def get_by_id(session: AsyncSession, task_id: int) -> Task | None:
        ...
        try:
            ...
            logger.error("Database error getting task by id: %s", e)
            raise
    ...
    async def update(session: AsyncSession, task: Task, completed: bool) -> Task:
        ...
        try:
            ...
            await session.rollback()
            logger.error("Database error updating task: %s", e)
            raise
    ...
    async def delete(session: AsyncSession, task: Task) -> None:
        ...
        try:
            ...
            await session.rollback()
            logger.error("Database error deleting task: %s", e)
            raise
```
...
Includes comprehensive error handling for all database operations.
"""
import logging
from typing import Sequence
...
class TaskService:
    ...
    async def create_task(session: AsyncSession, title: str) -> Task:
        ...
        Args:
            session: Database session.
            title: Task description (validated and sanitized by Pydantic schema).
        ...
        try:
            ...
            logger.error("Integrity error in create_task: %s", e)
            raise
        except SQLAlchemyError as e:
            logger.error("Database error in create_task: %s", e)
            raise
    ...
    async def get_all_tasks(session: AsyncSession) -> Sequence[Task]:
        ...
        try:
            ...
            logger.error("Database error in get_all_tasks: %s", e)
            raise
    ...
    async def update_task(session: AsyncSession, task_id: int, completed: bool) -> Task:
        ...
        try:
            ...
            logger.error("Integrity error in update_task: %s", e)
            raise
        except SQLAlchemyError as e:
            logger.error("Database error in update_task: %s", e)
            raise
    ...
    async def delete_task(session: AsyncSession, task_id: int) -> None:
        ...
        try:
            ...
            logger.error("Database error in delete_task: %s", e)
            raise
```
...
All endpoints use Pydantic schemas for input validation and output serialization.
"""
import logging
from typing import Sequence
...
@router.post(
    ...
    description="Creates a new TODO task with the provided title. Title is sanitized to prevent XSS.",
)
async def create_task(
    ...
    Args:
        task_data: Validated and sanitized task creation data (title).
...
Includes comprehensive error handling for all exception types.
"""
import logging
...
async def integrity_error_handler(request: Request, exc: IntegrityError) -> JSONResponse:
    ...
    Returns:
        JSON response with 409 Conflict status.
    """
    logger.error("IntegrityError handled: %s", exc)
    return JSONResponse(
        status_code=409,
...
async def sqlalchemy_error_handler(request: Request, exc: SQLAlchemyError) -> JSONResponse:
    ...
    """
    logger.error("SQLAlchemyError handled: %s", exc, exc_info=True)
    return JSONResponse(
        status_code=500,
...
@pytest.mark.asyncio
async def test_create_task_xss_sanitization(client: AsyncClient):
    """Test that XSS attempts in title are sanitized."""
    response = await client.post(
        "/api/v1/tasks",
        json={"title": "<script>alert('xss')</script>Купить хлеб"},
    )
    ...
    assert "alert" not in data["title"]
    assert "Купить хлеб" in data["title"]
```
...
@pytest.mark.asyncio
async def test_concurrent_update_race_condition(client: AsyncClient):
    """
    Test for race conditions when multiple requests update the same task concurrently.
    
    This test verifies that concurrent updates to the same task don't cause
    data corruption or unexpected behavior.
    """
    # Create a task
    create_response = await client.post("/api/v1/tasks", json={"title": "Race condition test"})
    task_id = create_response.json()["id"]

    # Simulate concurrent updates
    async def toggle_task(completed: bool):
    ...
    # All requests should succeed (200) or one might get a conflict
    # but none should cause 500 errors
    status_codes = [r.status_code for r in results]
    assert all(code in [200, 409] for code in status_codes), f"Unexpected status codes: {status_codes}"

    # Final state should be consistent - task should exist
...
- Защита от XSS-атак (санитизация HTML)
